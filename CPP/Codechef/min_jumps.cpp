...
*                          `=---='
*/
//https://practice.geeksforgeeks.org/problems/minimum-number-of-jumps-1587115620/1?page=1&difficulty[]=1&curated[]=1&curated[]=8&sortBy=submissions
...
int findMaxIdx(int arr[], int i, int maxJump, int n) {
	int j = i + 1;
	int check = arr[j + 1];
	int ans = j;
	while (j <= n - 1 && j <= maxJump) {
		ans = check > arr[ans] ? j : ans;
		j++;
	}
	return ans;
}
...
int solve(int arr[], int n) {
	int jump = 0, nxtIdx = 1, i = 0;
	if (arr[0] == 0) {
		return -1;
	}
	while (i < n - 1) {
		long long maxJump = i + arr[i];
		nxtIdx = findMaxIdx(arr, i, maxJump, n);
		i += nxtIdx;
		jump++;
	}
	return jump;
