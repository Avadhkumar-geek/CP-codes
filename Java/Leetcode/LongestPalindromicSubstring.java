...
public class LongestPalindromicSubstring {
    ...
    public static String longestPalindrome(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            int lngst = Math.min(i, s.length() - i + 1);
            if (ans.length() > lngst * 2) {
                return ans;
            }
            String curr = availPal(s, i - 1, i + 1);
            if (curr.length() > ans.length()) {
                ans = curr;
            }
            curr = availPal(s, i, i + 1);
            if (curr.length() > ans.length()) {
                ans = curr;
            }
        }
    ...
    public static String availPal(String str, int l, int r) {
